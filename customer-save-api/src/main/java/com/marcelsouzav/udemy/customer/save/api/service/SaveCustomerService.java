...
public class SaveCustomerService {
    ...
    public String execute(CustomerJson customerJson) throws ExecutionException, InterruptedException, JsonProcessingException {
        ...
        // montando o producer que ira ser enviado para o kafka //configuro para aonde eu envio e para aonde eu recebo
        ProducerRecord<String, String> record = new ProducerRecord<String, String>(requestTopic, jsonString); //pra aonde eu to enviando
        record.headers().add(new RecordHeader(KafkaHeaders.REPLY_TOPIC, requestReplyTopic.getBytes())); //pra aonde eu vou receber

        // enviando
        RequestReplyFuture<String, String, String> sendAndReceive = kafkaTemplate.sendAndReceive(record); //esse metodo executa
